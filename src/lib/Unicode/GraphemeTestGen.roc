## The purpose of this file is to generate the Unicode.GraphemeTest.roc test suite.
##
...
import Unicode.Helpers exposing [hexBytesToU32]
import Unicode.CodePoint exposing [CodePoint]
...
main =
    when Arg.list! |> List.get 1 is
        Err _ -> Task.err (InvalidArguments "USAGE: roc run Unicode.GraphemeTest.roc -- path/to/package/")
        Ok arg -> File.writeUtf8 "$(Unicode.Helpers.removeTrailingSlash arg)/Unicode.GraphemeTest.roc" template
...
template =
    ...
    tests =
        ...
        |> List.map \test ->
            ...
            |> List.join
            |> Inspect.toStr)
                    |> List.map Unicode.InternalCP.fromU32Unchecked
                    |> Unicode.CodePoint.toStr
                    |> Result.try Unicode.Grapheme.split
    ...
    toCodePointList = \\strings ->
        strings |> List.map \\str ->
            when str |> Str.toUtf8 |> Unicode.CodePoint.parseUtf8 is
                Ok cps -> List.map cps Unicode.CodePoint.toU32
...
toU32List = \tokens ->
    ...
    toU32s = \cps -> cps |> List.map Unicode.CodePoint.toU32
