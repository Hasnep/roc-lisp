...
import Unicode.CodePoint exposing [CodePoint, Utf8ParseErr]
import Unicode.InternalGBP exposing [GBP]
import Unicode.InternalCP exposing [fromU32Unchecked]
import Unicode.InternalEmoji

## Extended Grapheme Cluster
Grapheme : Unicode.InternalGBP.GBP
...
split = \str ->
    ...
    codePoints <- str |> Str.toUtf8 |> Unicode.CodePoint.parseUtf8 |> Result.map

    breakPoints = codePoints |> List.map Unicode.InternalGBP.fromCP
...
toListStr = \tokens ->
    ...
    |> List.map \cpList ->
        # TODO DISCUSS
        # Again I'm not sure if we should crash here... I dont think Unicode.CodePoint.toStr
        # should be returning a result... to be discussed.
        when Unicode.CodePoint.toStr cpList is
            Ok str -> str
...
splitHelp = \state, codePoints, breakPoints, acc ->
    ...
    when (state, codePoints, breakPoints) is
        ...
        (LastWithPrev prev, [cp], [bp]) if bp == ZWJ ->
            if prev |> Unicode.CodePoint.toU32 |> Unicode.InternalEmoji.isPictographic then
        ...
        (EmojiSeqNext prev, [cp, ..], [_, ..]) ->
            if cp |> Unicode.CodePoint.toU32 |> Unicode.InternalEmoji.isPictographic then
        ...
        _ ->
            crash
                ...
                Unicode.Grapheme.split state machine state at the time was:
                $(Inspect.toStr (state, List.map codePoints Unicode.CodePoint.toU32, breakPoints))
                """
